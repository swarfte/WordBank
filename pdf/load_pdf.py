...
old_choice = re.compile(r"[0-9]{1,2}\.\s{2}[a-zA-Z]+(\s[a-zA-Z]+)*|[0-9]{3}\.\s[a-zA-Z]+(\s[a-zA-Z]+)*")
word_bank_4 = re.compile(r"[0-9]{1,3}\s[a-zA-Z]+(\s[a-zA-Z]+)*")
word_bank_5 = re.compile(r"[0-9]{1,3}\.\s[a-zA-Z]+(\s[a-zA-Z]+)*")

def get_pdf_data(path, rule="default"):  # *選取pdf檔案中的英文規則
    native_word = []
    choice = re.compile(r"[0-9]{1,3}\.\s[a-zA-Z]+(\s[a-zA-Z]+)*")
    word = []
    if rule == "default":
...
if __name__ == "__main__":
    pdf_file = "ch5.pdf"
    english_word = get_pdf_data(pdf_file, rule=word_bank_5)
